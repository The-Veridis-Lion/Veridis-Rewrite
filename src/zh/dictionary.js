/*
 * Owns OpenCC dictionary-package settings normalization, cache/download/integrity verification,
 * and installation of the verified runtime dictionary index. It does not own Rule matching or text conversion behavior.
 */
...
export const MAX_VARIANTS_PER_TARGET = 96;
...
function setRuntimeDictionary(packagePayload) {
    ...
    zhRuntimeState.zhVariantDictionary = {
        ready: true,
...
export function restoreZhDictionaryPackageFromCache(settings = getSettings()) {
    normalizeZhVariantSettings(settings);
    if (zhRuntimeState.zhVariantDictionary?.ready === true
        && zhRuntimeState.zhVariantDictionary.packageVersion === ZH_DICTIONARY_PACKAGE_VERSION) {
        updateSettingsDictionaryMeta({
            digest: zhRuntimeState.zhVariantDictionary.digest || '',
            verifiedAt: zhRuntimeState.zhVariantDictionary.verifiedAt,
        }, 'verified', '', settings);
        return true;
    }
...
export function isZhDictionaryRuntimeReady() {
    return zhRuntimeState.zhVariantDictionary?.ready === true
        && zhRuntimeState.zhVariantDictionary.packageVersion === ZH_DICTIONARY_PACKAGE_VERSION;
}

export function isZhDictionaryReady(settings = getSettings()) {
    if (isZhDictionaryRuntimeReady()) {
        normalizeZhVariantSettings(settings);
        updateSettingsDictionaryMeta({
            digest: zhRuntimeState.zhVariantDictionary.digest || '',
            verifiedAt: zhRuntimeState.zhVariantDictionary.verifiedAt,
