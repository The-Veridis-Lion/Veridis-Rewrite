...
import { aiRewritePromptProtocolVersion, applyGemini31TemperatureMigration, defaultAiRewriteSettings, defaultSettings, extensionName, modifiedExtensionName, legacyExtensionName, isKnownBuiltInAiRewritePrompt, migrateKnownAiRewritePrompt, needsCustomGlobalPromptMigrationConfirmation, normalizeAiSamplingSettings, normalizeDiffTrackedMessageLimit, normalizeShujukuAutoProgramRewriteEnabled, resolveCustomGlobalPromptMigration } from './defaults.js';
import { markRulesDataDirty } from '../rules/state.js';
import { logger } from '../log.js';
import { cleanupInvalidPresetBindings } from '../presets/bindings.js';
import { buildPresetEntry, getCurrentPresetAiRewriteSettings, getPresetAiRewriteSettings, getPresetRules } from '../presets/model.js';
import { normalizeRuleActivationSafety } from '../rules/model.js';
import { mergeScopeTagsWithBuiltins, normalizeOptionalXmlTagNameInput, normalizeScopeTagBuiltinDismissedList, normalizeScopeTagCollapsedGroupList, normalizeScopeTagGroupList } from '../scope/model.js';
import { normalizeZhVariantSettings, restoreZhDictionaryPackageFromCache } from '../zh/dictionary.js';

const { extension_settings } = extensionsModule;

export let legacySettingsCopiedThisBoot = false;
export let modifiedSettingsImportedThisBoot = false;
...
function normalizeAiRewriteSettings(settings) {
    ...
    delete next.maxRewriteCharsPerItem;
    ...
    next.gemini31TemperatureMigrationApplied = current.gemini31TemperatureMigrationApplied === true;
    applyGemini31TemperatureMigration(next);
    delete next.promptScope;
    const savedPromptTemplate = String(current.promptTemplate || '');
    next.promptTemplate = migrateKnownAiRewritePrompt(savedPromptTemplate) || defaultAiRewriteSettings.promptTemplate;
    next.promptProtocolVersion = aiRewritePromptProtocolVersion;
    next.globalPromptMigrationDecisionMade = !savedPromptTemplate
        || isKnownBuiltInAiRewritePrompt(savedPromptTemplate)
        || savedPromptTemplate === defaultAiRewriteSettings.promptTemplate
        || current.globalPromptMigrationDecisionMade === true;
...
export function maybeImportModifiedSettingsIntoSharedNamespace() {
    ...
    modifiedSettingsImportedThisBoot = true;
...
export { needsCustomGlobalPromptMigrationConfirmation, resolveCustomGlobalPromptMigration };
...
function normalizePresetEntriesShape(settings) {
    ...
    Object.keys(settings.presets).forEach((name) => {
        ...
        const currentAiRewrite = presetEntry?.aiRewrite;
        const migratedAiRewrite = currentAiRewrite && typeof currentAiRewrite === 'object'
            ? {
                ...currentAiRewrite,
                promptTemplate: migrateKnownAiRewritePrompt(currentAiRewrite.promptTemplate) || defaultAiRewriteSettings.promptTemplate,
        ...
        settings.presets[name] = buildPresetEntry(
            rules,
            getPresetAiRewriteSettings({ ...presetEntry, aiRewrite: migratedAiRewrite })
                || getCurrentPresetAiRewriteSettings(settings.aiRewrite)
        );
    });
}
