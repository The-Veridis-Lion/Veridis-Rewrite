...
- **新增插件版本信息**：工具页可查看当前版本、更新内容与更新状态，并提供“更新并刷新”入口。
- **新增更新提醒**：区分版本升级与小型修复更新；发现可用更新时，会在反馈页面提醒先更新。
- **聊天改写顺序调整**：先由 AI 根据原文改写命中句子，再执行规则净化，统一生成最终结果。
- **改写提示与引号处理优化**：改善命中句子的表达和衔接，减少对话引号重复等问题；旧版内置提示词自动更新，自定义提示词仍由用户决定是否替换。
- **随机替换行为统一**：配置多个替换候选时，各次命中独立随机选择，改善同类表达总出现固定替换的情况。
- **差异记录优化**：按照实际处理顺序展示 AI 改写、规则净化和手动修改，改善记录保留与查看体验。
- **反馈信息完善**：补充版本识别信息，并完善已启用扩展的信息读取，方便排查问题。
- **使用体验优化**：优化界面布局与常用操作体验，修复部分问题。
...
- 支持一条规则配置多个目标词和多个替换候选，每次命中独立随机选择；没有替换候选时会直接删除命中内容。
...
- 普通聊天改写先根据原文进行 AI 改写，再执行规则净化；最终内容与差异记录准备完成后，再统一更新消息。
- 处理中显示命中数量和实际请求批次，不把自动重试次数暴露为进度；可随时点击“终止”。
- 提供诊断日志，记录请求分组、命中数、重试原因、最终净化和原子提交阶段，方便排查未发送、超时或返回格式错误。

### 实时净化

- 流式阶段由 SillyTavern 先提交并渲染累计 Original，Veridis 随后计算并保留 Program，仅在结果与 Original 不同时进行视觉投影；正式消息修改与持久化仍由终稿流程负责。
...
- “工具”：查看插件版本与更新内容、执行更新，并管理预设绑定解析、输入框 AI Rewrite 入口、Diff 保留范围、可选 Shujuku 兼容及 OpenCC 设置。
...
```powershell
git archive --format=zip --prefix=Veridis-Rewrite/ -o Veridis-Rewrite-3.3.zip HEAD README.md index.js manifest.json style.css images src styles templates vendor
```
